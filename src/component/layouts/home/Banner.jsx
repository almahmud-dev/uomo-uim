...
const Banner = () => {
  ...
  <SwiperSlide key={idx}>
    <div
      style={{ backgroundImage: `url('https://res.cloudinary.com/dlqvctrgm/image/upload/q_auto/f_auto/v1775147827/bannerBg_waubaz.png')` }}
      ...
      <div className="container px-4 sm:px-7.5 xl:px-0 h-full relative">
        <div className="leftContent absolute z-10 bottom-12 sm:bottom-20 lg:bottom-auto lg:top-[50%] lg:-translate-y-[10%] left-4 sm:left-7.5 xl:left-0 w-[52%] sm:w-[50%] lg:w-[45%]">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <p className="texts_14_medium text-second-red text-[10px] sm:text-[12px] lg:text-[14px]">NEW TREND</p>
          </div>
          ...
          <div className="mt-6 sm:mt-10 lg:mt-16 flex gap-1">
            {sliderData.map((_, index) => (
              <div onClick={() => swiperRef.current?.slideToLoop(index)} key={index} className="group relative flex items-center justify-center p-2 sm:p-3 cursor-pointer">
                <div className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${activeIndex === index ? "bg-black" : "bg-[#DDC2B9] group-hover:bg-black"}`}></div>
                <div className={`absolute inset-0 border-2 border-black rounded-full transition-all duration-300 ${activeIndex === index ? "scale-100" : "scale-0 group-hover:scale-100"}`}></div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="absolute top-1/2 -left-10 -translate-y-[50%] hidden lg:block">
          <div className="flex flex-col gap-y-6.25 z-10">
            ...
            <Link href="#"><p className="texts_14_medium text-second rotate-270 -ml-7.75 mt-6.5">FOLLOW US</p></Link>
          </div>
        </div>
